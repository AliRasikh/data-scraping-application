"""
This module provides a Flask web application for scraping websites and
managing user authentication and history.
It includes endpoints for scraping a website using different methods,
user authentication, and managing user history.

Endpoints:
- /auth (GET): Verifies the JWT token.
- /scrape (POST): Scrapes a website using the specified method (requests, bs4, or selenium)
...
def token_required(func):
    ...
    Raises:
        401 Unauthorized: If the token is missing, has an invalid format,
          is expired,
...
def scrape():
    ...
    - "scraping_method": The method to use for scraping,
      either "requests", "bs4",
    or "selenium" (required).
